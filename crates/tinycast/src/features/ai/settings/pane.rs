...
use tinycast_pure::ai::{AiConnection, ModelSelection, ProviderKind};
...
pub fn content_height(connection_count: usize) -> f32 {
    24.0 + (FIXED_ROWS + connection_count) as f32 * (ROW_H + theme::spacing::XL)
}
...
pub fn hit(x: f32, y: f32, scroll: f32, connection_count: usize, width: f32) -> Option<AiHit> {
    let y = y + scroll;
