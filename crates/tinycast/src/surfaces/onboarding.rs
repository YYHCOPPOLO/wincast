//! First-launch window: record `togglePalette`, then continue into the launcher.
...
use windows::Win32::UI::WindowsAndMessaging::{
    ...
    WM_CLOSE, WM_COMMAND, WM_DESTROY, WM_KEYDOWN, WM_NCCREATE, WM_NCDESTROY, WM_PAINT, WNDCLASSW,
...
unsafe extern "system" fn wndproc(hwnd: HWND, msg: u32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    match msg {
        ...
        WM_KEYDOWN => {
            if inner_from(hwnd).map(|i| (*i).recording).unwrap_or(false) {
                if let Some(binding) = capture_keydown(wparam.0 as u16) {
                    if let Some(inner) = inner_from(hwnd) {
...
fn capture_keydown(vk: u16) -> Option<HotKeyBinding> {
    ...
    Some(HotKeyBinding::Combo(KeyShortcut { vk, modifiers }))
}
